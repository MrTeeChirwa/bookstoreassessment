<template>
  <div class="checkout-box">
    ...
    <div class="btn-order" v-if="hasProduct()">
      <button class="btn" @click="placeOrder">Order Now!</button>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    totalPrice() {
      return this.getProductsInCart.reduce((current, next) =>
        parseInt(next.price) + current, 0);
    },
    placeOrder(){
      axios.post("http://localhost:3000/create-order", this.getProductsInCart).then(response => {
